...
This vignette shows to to run a `TreatmentPatterns` analysis using
`Strategus`.

> The `Strategus` package is a new approach for coordinating and
> executing analytics using HADES modules. The goal is to have OHDSI
> network sites install Strategus and exchange an analysis specification
> in JSON format to execute a network study. The analysis specification
> will capture all of the design choices that pertain to the methods
> used in a given study. The analysis specification format aims to allow
> for combining different HADES modules together as a pipeline to
> execute a study.
...
First we specify the execution settings, which dictate how to connect to
the database.
...
Secondly we need need to tell Strategus which cohorts to generate, and
where.
